...
async function runUpdate(isForce) {
    let command = "git -C ./plugins/btc-memz-plugin/ pull --no-rebase";
    if (isForce) {
        command = `git -C ./plugins/btc-memz-plugin/ checkout . && ${command}`;
        that.e.reply("正在执行强制更新操作，请稍等");
    } else {
        that.e.reply("正在执行更新操作，请稍等");
    }
    /** 获取上次提交的commitId，用于获取日志时判断新增的更新日志 */
    that.oldCommitId = await getcommitId("btc-memz-plugin");
    uping = true;
    let ret = await exectSync(command);
    uping = false;

    if (ret.error) {
        logger.mark(`${that.e.logFnc} 更新失败：btc-memz-plugin`);
        gitErr(ret.error, ret.stdout);
        return false;
    }

    /** 获取插件提交的最新时间 */
    let time = await getTime("btc-memz-plugin");

    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
        await that.reply(`btc-memz-plugin已经是最新版本\n最后更新时间：${time}`);
    } else {
        await that.reply(`btc-memz-plugin\n最后更新时间：${time}`);
        that.isUp = true;
        /** 获取千羽组件的更新日志 */
        let log = await getLog("btc-memz-plugin");
...
async function getLog(plugin = "") {
    ...
    log = await makeForwardMsg(`btc-memz-plugin更新日志，共${line}条`, log, end);
